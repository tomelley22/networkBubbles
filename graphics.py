...
#     Made Polygon accept a list of Vectors in constructor
...
class GraphWin(tk.Canvas):
    ...
    def getMouse(self):
        """Wait for mouse click and return Vector object representing
        the click"""
    ...
    def _onClick(self, e):
        ...
        if self._mouseCallback:
            self._mouseCallback(Vector(e.x, e.y))
...
class Circle(Oval):
    ...
    def __init__(self, center, radius):
        p1 = Vector(center.x-radius, center.y-radius)
        p2 = Vector(center.x+radius, center.y+radius)
...
class Polygon(GraphicsObject):
    ...
    def __init__(self, *points):
        ...
        if len(points) == 1 and type(points[0]) == type([]):
            points = points[0]
        self.points = list(map(Vector.clone, points))
    ...
    def getVectors(self):
        return list(map(Vector.clone, self.points))
...
def test():
    ...
    t = Text(Vector(5,5), "Centered Text")
    t.draw(win)
    p = Polygon(Vector(1,1), Vector(5,3), Vector(2,7))
    p.draw(win)
    e = Entry(Vector(5,6), 10)
    e.draw(win)
    win.getMouse()
    p.setFill("red")
    p.setOutline("blue")
